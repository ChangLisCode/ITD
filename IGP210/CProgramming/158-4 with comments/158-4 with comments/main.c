...
/*
◊¢ Õ£∫’‚¿Ôµƒº∏∏ˆ∫Ø ˝√˚◊÷∆µƒ±»ΩœÀÊ“‚£¨“ª∞„∫Ø ˝√˚º∞±‰¡ø√˚◊Ó∫√∂º «◊‘Ω‚ Õµƒ£¨º¥ø…“‘∏˘æ›√˚≥∆ø¥≥ˆ∆‰∫¨“Â£¨
 */
...
int main(void) {
    ...
    /*◊¢ Õ£∫ ‘⁄À˘”–µ˜”√ malloc ÷Æ∫Ûµƒµÿ∑Ω£¨∂º–Ë“™œ»≈–∂œœ¬…Í«Îµƒ÷∏’Î «∑ÒŒ™ø’£¨∑¿÷π…Í«Î≤ªµΩƒ⁄¥Êµº÷¬≥Ã–Ú“Ï≥£
    eg£∫
    if(date == NULL)
    {
        printf("date malloc error,please check if we have enough memory to use");
        return -1;
    }
    */
    ...
    /*◊¢ Õ£∫ ‘⁄À˘”–µ˜”√ free÷Æ«∞µƒµÿ∑Ω£¨∂º–Ë“™œ»≈–∂œœ¬ Õ∑≈µƒ÷∏’Î «∑ÒŒ™ø’£¨∑¿÷π÷ÿ∏¥ Õ∑≈µº÷¬≥Ã–Ú“Ï≥£
    eg £∫
    if(date != NULL)
        free(date);
    */
...
/* ◊¢ Õ£∫new_product ◊Ó∫√∏ƒ≥… new_product_cnt, ∑Ω±„¿ÌΩ‚ */
void import_information(int new_product, int date[]){
    
    /* ◊¢ Õ£∫
    struct Product *products[new_product] ˝◊È¥Û–° π”√±‰¡ø¿¥∂®“Â£¨’‚÷÷”√∑® «≤ªÕ∆ºˆµƒ
    Ω®“È◊ˆ∑® «£¨∫Õ∫Û√Ê∑÷≈‰Ω·ππÃÂƒ⁄¥Ê“ª∆
    eg£∫
    
    struct Product *products = (struct Product*) malloc(sizeof(struct Product) * new_product)
    ’‚—˘£¨‘⁄œ¬√Êµƒfor—≠ª∑ƒ⁄≤ø£¨æÕ≤ª–Ë“™‘Ÿ…Í«Îƒ⁄¥Ê
    ‘⁄’‚∏ˆ∫Ø ˝◊Ó∫Û√Ê£¨ Õ∑≈ƒ⁄¥Ê ±£¨÷±Ω”£∫ free(products); º¥ø…
...
//      go to shopping part
        ...
        /* ◊¢ Õ£∫
            struct Product *shopping_list[amount]∫Õ∏√∫Ø ˝»Îø⁄¥¶
            struct Product *products[new_product]¥Ê‘⁄µƒŒ Ã‚¿‡À∆£¨ø…“‘≤Œ’’Ω¯–––ﬁ∏ƒ
        */
        ...
        for (i=0; i < amount; i++) {
            for (j = 0; j < 36; j++) {
                if (strcmp(shopping_list[i]->barcode, products[j]->barcode) == 0) {
                    /* ◊¢ Õ£∫π§≥Ã÷–£¨¥Û∂‡ π”√ strncpy ¥˙ÃÊ strcpy£¨ø…“‘∑¿÷πøΩ±¥π˝≥Ã÷–ƒ⁄¥Ê‘ΩΩÁ */
...
/* ◊¢ Õ£∫’‚∏ˆ∫Ø ˝µƒ≤Œ ˝±»Ωœ∂‡£¨ø…“‘øº¬«÷±Ω”≤…”√“ª∏ˆΩ·ππÃÂµƒ÷∏’Î¥´µ›Ω¯¿¥£¨–ß¬ ∏¸∏ﬂ */
char barcode_generate(char *name, char *group, int purchased_date, struct Product* products){
...
/*
◊¢ Õ£∫’‚∏ˆ∫Ø ˝µƒ∂®“ÂªÚ’ﬂ π”√¥Ê‘⁄Œ Ã‚£¨»Áπ˚À˚µƒ∑µªÿ÷µ « char * ,ƒ«√¥µ˜”√µƒ“≤”¶∏√ « π”√ char * µƒ ˝æ›¿‡–Õ¿¥Ω” ’
eg£∫
char * encrypted_str = *encryption_method_1("meat");
»ª∫Û‘⁄∞— encrypted µƒµ⁄“ª∏ˆ◊÷ƒ∏‘Ÿ◊™ªª≥… ˝◊÷
int encrypted_num = (int )(*encrypted_str);
*/
char* encryption_method_1(char string[]){
